...
### BioE Interview summary (Mar 6, 2026 Haotian and John)

**_TL;DR Proposal:**_
	**Research question**: does STS/STG encode a different spectrogram of auditory-only speech compared to audiovisual speech (w/o noise, incongruent/congruent)?
	**Methods**: Generative speech enhancement models are trained to identify speech  from noise-corrupted signals (usually spectrogram as the input) and restore clear speech signals by generating an enhanced spectrogram of estimated speech without noise. This process can be combined with visual inputs (mouth ROI) as the visual component will keep adjusting the posteriori (i.e. enhanced speech). 
	We will present the model speech of different types (A/V/AV * congr/incongr * noise/clear) and compare the reconstructed speech signals with the electrophysiological signals in STS/STG to see whether reconstructed spectrogram explains brain representations. And we can do this in both encoding and decoding directions.

We have found from Yue's paper + new iEEG paper + fMRI MVPA that the STS/STG An+V=Ac

***Details:***
Conditions:
 1. Encoding model (STRF) trained on auditory-only speech signals or on av signals: does STRF trained on a-o speech predicts Heschl's gyrus better than STS/STG? What about STRF trained on AV spectrograms? 
 2. We want to see if An+V=Ac holds
 3. We want to see if Aba+Vba=Aba and Aba+Vga = AbaVga != Aba != Vga != Ada 
 
Stimuli:
1. MECOM sentences 
2. The generative model might predict clean speech very well (without using much of the visual information) because clean speech from one speaker + noise is very simple for machines. We may consider corrupt the source in a certain way that can be explained in the context of perception pathways. For instance: 
- [ ] instead of using the spectrogram, use cochleagram (or some filtered speech spectrogram) to add time-frequency ambiguity. in this way we might have a clear condition (filtered by cochlear) that actually works with the models and multiple noisy condition (cochleagram + noise)


Model:
Try fast_UdiffSE (https://github.com/T4phage76/fast_UdiffSE/tree/main) first and find/train shallower (more interpretable) models. 



## Extra notes: ##
compare STS vs. STG vs. Heschl's Gyrus

https://www.isca-archive.org/interspeech_2019/qu19_interspeech.html

https://arxiv.org/html/2505.08694v2

https://pmc.ncbi.nlm.nih.gov/articles/PMC12962650/
