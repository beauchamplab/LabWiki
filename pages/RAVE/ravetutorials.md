---
title: ravetutorials
parent: RAVE
---
# ravetutorials

|  |  |
| --- | --- |
| [RAVE logo](../../attachments/RAVE/RAVE_Logo_new.jpg "RAVE logo") | **R**eproducible **A**nalysis and **V**isualization of i**E**EG   ***RAVE*** |

## Contents

* [1 Class Tutorials](#Class_Tutorials)
* [2 Text Tutorials](#Text_Tutorials)
* [3 Video Tutorials](#Video_Tutorials)
  + [3.1 Reference Module](#Reference_Module)
  + [3.2 Power Explorer Module](#Power_Explorer_Module)

## Class Tutorials

[2021 Computational Memory Lab's Workshop on Cognitive Electrophysiology](CML_Workshop_2021.md "RAVE:CML Workshop 2021")

## Text Tutorials

The first step in using RAVE for your own data (instead of the included sample data) is [preprocessing](ravepreprocess.md "RAVE:ravepreprocess").

[Click here for the electrode localization tutorial.](../Karas_Lab/RAVE_Electrode_Localization.md "Karas Lab:RAVE Electrode Localization")

[Click here for the tutorial on importing data.](Tutorial_Importing.md "RAVE:Tutorial Importing")

[Click here for tutorial on preprocessing data.](Tutorial_Preprocessing.md)

[Click here for the tutorial on importing FreeSurfer data.](Tutorial_ImportFS.md "RAVE:Tutorial ImportFS")

[Click here for the tutorial on referencing signals.](Tutorial_Referencing.md "RAVE:Tutorial Referencing")

[Click here for the tutorial on analyzing subjects's data in the Power Explorer.](Tutorial_PowerExplorer.md)

[Click here for the tutorial on RAVE Clusters.](Tutorial_Clusters.md)

## Video Tutorials

### Reference Module

Load videoYouTubeYouTube might collect personal data. [Privacy Policy](https://www.youtube.com/howyoutubeworks/user-settings/privacy/)

Continue
Dismiss

### Power Explorer Module

Load videoYouTubeYouTube might collect personal data. [Privacy Policy](https://www.youtube.com/howyoutubeworks/user-settings/privacy/)

Continue
Dismiss
